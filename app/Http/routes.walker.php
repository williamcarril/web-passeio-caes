...
Route::group(["middleware" => "auth.walker"], function() {
    ...
    Route::group(["prefix" => "passeio"], function() {
        ...
        Route::get("/{id}", ["as" => "walker.passeio.detalhes.get", "uses" => "PasseioController@route_getWalkerPasseio"])
                ->where('id', '[0-9]+');
        Route::get("/{id}/rota", ["as" => "walker.passeio.rotas.get", "uses" => "PasseioController@route_getWalkerPasseioRotas"])
                ->where('id', '[0-9]+');;
    ...
    Route::group(["prefix" => "local"], function() {
        Route::get("/", ["as" => "walker.local.listagem.get", "uses" => "LocalController@route_getWalkerLocais"]);
        Route::get("/{id}", ["as" => "walker.local.detalhes.get", "uses" => "LocalController@route_getWalkerLocal"])
            ->where('id', '[0-9]+');
    });
});
